...
@app.route("/add_user2", methods=['GET','POST']) # last bit tells what methods are allowed
def add_user2():
    ...
    if form.validate_on_submit():
        ...
        if checkemail((email,),allemails) == False:
            ...
            cursor.close()
            flash(f"Account created for {form.firstname.data} {form.lastname.data}!", 'success')
            return render_template('add_user2.html', form=form)
        else :
            yocursor = connection.cursor()
            select_stmt = "SELECT * from Users WHERE company_email = %(company_email)s"
            yocursor.execute(select_stmt, { 'company_email': email })
            ...
            connection.commit()
            yocursor.close()
            flash(f"the email address {form.email.data} is already used!", 'danger')
            return render_template('add_user2.html', form=form, datav=data)
    return render_template('add_user2.html', form=form)
...
#cursor = connection.cursor()
#cursor.execute("INSERT INTO Users(first_name, last_name, company_email) VALUES (%s, %s, %s)",
#               ({{form.firstname.label}}, {{form.firstname.label}}, {{form.firstname.label}}))
#connection.commit()
#cursor.close()
...
#@app.route('/add_user2', methods=['GET', 'POST'])
#def signup():
#
...
#            cursor.close()
#            msg = 'user added to DB :)'
#            return render_template('add_user2.html', msg=msg)
#        else :
#            yocursor = connection.cursor()
#            select_stmt = "SELECT * from Users WHERE company_email = %(company_email)s"
#            yocursor.execute(select_stmt, { 'company_email': email })
...
#            yocursor.close()
#            msg = 'email already in DB :('
#            return render_template('add_user2.html', msg=msg, datav=data)
